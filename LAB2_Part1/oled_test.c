...
for(y=0; y <= (height()-1); y=y+5) {

        for(x=0; x <= (width()-1); x=x+5) {
        drawChar(x, y, font[i], BLUE, BLUE, 1);
        i=i+1;
        if(i<1274){
            return;
        }
...
void Hello_World(){
    fillScreen(BLACK);
    drawChar(0,64, 'H', WHITE, WHITE, 1);
    drawChar(8,64, 'E', WHITE, WHITE, 1);
    drawChar(16,64, 'L', WHITE, WHITE, 1);
    drawChar(24,64, 'L', WHITE, WHITE, 1);
    drawChar(32,64, 'O', WHITE, WHITE, 1);
    drawChar(40,64, ' ', WHITE, WHITE, 1);
    drawChar(48,64, 'W', WHITE, WHITE, 1);
    drawChar(56,64, 'O', WHITE, WHITE, 1);
    drawChar(64,64, 'R', WHITE, WHITE, 1);
    drawChar(72,64, 'L', WHITE, WHITE, 1);
    drawChar(80,64, 'D', WHITE, WHITE, 1);
    drawChar(88,64, '!', WHITE, WHITE, 1);
...
void eight_h(unsigned int color1,unsigned int color2,unsigned int color3,unsigned int color4,unsigned int color5,unsigned int color6,unsigned int color7,unsigned int color8){
    unsigned int x;
    unsigned int y;
    fillScreen(BLACK);

      delay(100);
      for (y=0; y < (height()-1)*1/8; y+=1) {
        drawFastHLine(0, y, width()-1, color1);
      }
        delay(100);
        for (y=(height()-1)*1/8; y < (height()-1)*2/8; y+=1) {
          drawFastHLine(0, y, width()-1, color2);
        }
        delay(100);
        for (y=(height()-1)*2/8; y < (height()-1)*3/8; y+=1) {
          drawFastHLine(0, y, width()-1, color3);
        }
        delay(100);
        for (y=(height()-1)*3/8; y < (height()-1)*4/8; y+=1) {
          drawFastHLine(0, y, width()-1, color4);
        }
        delay(100);
        for (y=(height()-1)*4/8; y < (height()-1)*5/8; y+=1) {
          drawFastHLine(0, y, width()-1, color5);
        }
        delay(100);
        for (y=(height()-1)*5/8; y < (height()-1)*6/8; y+=1) {
          drawFastHLine(0, y, width()-1, color6);
        }
        delay(100);
        for (y=(height()-1)*6/8; y < (height()-1)*7/8; y+=1) {
          drawFastHLine(0, y, width()-1, color7);
        }
        delay(100);
        for (y=(height()-1)*7/8; y < (height()-1)*8/8; y+=1) {
          drawFastHLine(0, y, width()-1, color8);
        }
        delay(100);
}
void eight_v(unsigned int color1,unsigned int color2,unsigned int color3,unsigned int color4,unsigned int color5,unsigned int color6,unsigned int color7,unsigned int color8){
    unsigned int x;
    unsigned int y;
    fillScreen(BLACK);

      delay(100);
      for (x=0; x < (width()-1)*1/8; x+=1) {
        drawFastVLine(x, 0, height()-1, color1);
      }
        delay(100);
        for (x=(width()-1)*1/8; x < (width()-1)*2/8; x+=1) {
          drawFastVLine(x, 0, height()-1, color2);
        }
        delay(100);
        for (x=(width()-1)*2/8; x < (width()-1)*3/8; x+=1) {
          drawFastVLine(x, 0, height()-1, color3);
        }
        delay(100);
        for (x=(width()-1)*3/8; x < (width()-1)*4/8; x+=1) {
          drawFastVLine(x, 0, height()-1, color4);
        }
        delay(100);
        for (x=(width()-1)*4/8; x < (width()-1)*5/8; x+=1) {
          drawFastVLine(x, 0, height()-1, color5);
        }
        delay(100);
        for (x=(width()-1)*5/8; x < (width()-1)*6/8; x+=1) {
          drawFastVLine(x, 0, height()-1, color6);
        }
        delay(100);
        for (x=(width()-1)*6/8; x < (width()-1)*7/8; x+=1) {
          drawFastVLine(x, 0, height()-1, color7);
        }
        delay(100);
        for (x=(width()-1)*7/8; x < (width()-1)*8/8; x+=1) {
          drawFastVLine(x, 0, height()-1, color8);
        }
        delay(100);
}
//*****************************************************************************
